...
function createEvent(overrides: Partial<BBGMEvent>): BBGMEvent {
  return {
    eid: 1,
    type: "generic",
    tids: [],
...
function createGame(overrides: Partial<BBGMGame>): BBGMGame {
  return {
    gid: 1,
    day: 1,
    season: 2024,
...
// Default test data
const defaultPlayers: BBGMPlayerMinimal[] = [
...
const defaultTeams: BBGMTeamMinimal[] = [
...
describe("stripHtmlToText", () => {
  it("strips anchor tags and extracts inner text", () => {
    ...
    expect(result).toBe(
      "The Hawks signed Zach Collins for $5.82M/year through 2022."
    );
  });
  ...
  it("handles game highlight text with multiple anchors", () => {
    ...
    const result = stripHtmlToText(html);
    expect(result).toBe(
      "Jayson Tatum had 41 points, 11 rebounds, and 5 assists in a 112-98 win over the Heat."
    );
  });
  ...
  it("handles trade text with multiple teams and players", () => {
    ...
    const result = stripHtmlToText(html);
    expect(result).toBe(
      "The Lakers traded Anthony Davis to the Celtics for Jaylen Brown."
    );
  });
...
describe("parseLeagueEvents - fallback text", () => {
  it("generates fallback text for freeAgent with no text", async () => {
    ...
    expect(result.parsedEvents[0].text).toBe(
      "Zach Collins signed with the Atlanta Hawks."
    );
  });
  ...
  it("generates fallback text for release with no text", async () => {
    ...
    expect(result.parsedEvents[0].text).toBe(
      "The Boston Celtics released Jayson Tatum."
    );
  });
  ...
  it("generates fallback text for trade with no text", async () => {
    ...
    expect(result.parsedEvents[0].text).toBe(
      "A trade was completed involving the Los Angeles Lakers and Boston Celtics."
    );
  });
});
