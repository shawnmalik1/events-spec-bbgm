 ...
 */
export function categorizeEvent(event: BBGMEvent): EventCategory {
...
export function generateFallbackText(
  ...
  const playerName =
    pids?.[0] !== undefined ? resolvePlayerName(pids[0], players) : null;
  const teamName =
    tids?.[0] !== undefined ? resolveTeamFullName(tids[0], teams) : null;
  const team2Name =
    tids?.[1] !== undefined ? resolveTeamFullName(tids[1], teams) : null;
 ...
 */
export function parseScoreFromText(text: string): ScoreParseResult | null {
...
export function findMatchingGame(
  ...
  for (const game of games) {
    if (game.season !== currentSeason) continue;
    ...
    if (eventTeamsInGame.length === 0) continue;
