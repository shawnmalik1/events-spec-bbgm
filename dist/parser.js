import { categorizeEvent, findMatchingGame, generateFallbackText, getTeamById, stripHtmlToText, } from "./utils.js";
// =============================================================================
 ...
 * 5. Assemble output
 */
export async function parseLeagueEvents(input) {
    ...
    const parsedEvents = [];
    const importantGameMap = new Map();
    for (const event of seasonEvents) {
        ...
        if (category === "game_highlight" &&
            ...
            if (matchedGame) {
                ...
                console.warn(`Could not match game for event ${event.eid}: "${text.substring(0, 50)}..."`);
            }
        }
        const parsedEvent = {
            eid: event.eid,
        ...
        if (associatedGameId !== null) {
            addToImportantGamesMap(importantGameMap, associatedGameId, parsedEvent, games, teams, currentSeason);
        }
    }
...
// Stage 1: Filter to Current Season
// =============================================================================
function filterToCurrentSeason(events, currentSeason) {
    return events.filter((event) => event.season === currentSeason);
}
// =============================================================================
// Stage 3: Parse Event Text
// =============================================================================
function parseEventText(event, players, teams) {
...
// =============================================================================
function addToImportantGamesMap(map, gid, parsedEvent, games, teams, currentSeason) {
    ...
    // Find the game data
    const game = games.find((g) => g.gid === gid && g.season === currentSeason);
    if (!game)
        return;
    // Get team info
