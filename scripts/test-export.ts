...
if (Array.isArray(gameAttributes)) {
  const seasonAttr = gameAttributes.find(
    (attr: { key: string }) => attr.key === "season"
  );
...
// Group by category
const byCategory = result.parsedEvents.reduce(
  (acc, e) => {
...
if (result.importantGames.length > 0) {
  ...
  for (const game of result.importantGames.slice(0, 5)) {
    ...
    console.log(
      `${game.winner.abbrev} ${game.winner.pts} - ${game.loser.pts} ${game.loser.abbrev}${playoff}`
    );
